...
using User = CookbookDB.Models.User;
...
public class RecipeRepositoryTests : IDisposable
{
    ...
    {
        var recipe1 = new RecipeModel { Name = "Tomato Soup", Instruction = "Cook", ServingsNumber = 2, UserId = TestUserId };
        var recipe2 = new RecipeModel { Name = "Potato Salad", Instruction = "Mix", ServingsNumber = 4, UserId = TestUserId };
        var recipe3 = new RecipeModel { Name = "Carrot Cake", Instruction = "Bake", ServingsNumber = 8, UserId = TestUserId };
    ...
    public async Task AddRecipeAsync_WhenValidData_CreatesRecipe()
    {
        ...
        var id = await _repository.AddRecipeAsync(recipeCreate, TestUserId);
        ...
        savedRecipe.UserId.Should().Be(TestUserId); // Проверка UserId
    ...
    public async Task AddRecipeAsync_WhenIngredientNotFound_ThrowsException()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() => _repository.AddRecipeAsync(recipeCreate, TestUserId));
    }
    ...
    public async Task AddRecipeAsync_WhenDuplicateIngredients_ThrowsException()
    {
        ...
        var exception = await Assert.ThrowsAsync<Exception>(() => _repository.AddRecipeAsync(recipeCreate, TestUserId));
    ...
    public async Task UpdateRecipeAsync_WhenRecipeExists_UpdatesRecipe()
    {
        ...
        await _repository.UpdateRecipeAsync(recipeUpdate, TestUserId);
        ...
        updatedRecipe.UserId.Should().Be(TestUserId); // UserId не должен меняться
    ...
    public async Task UpdateRecipeAsync_WhenRecipeDoesNotExist_ThrowsException()
    {
        ...
        var exception = await Assert.ThrowsAsync<Exception>(() => _repository.UpdateRecipeAsync(recipeUpdate, TestUserId));
    ...
    public async Task DeleteRecipeAsync_WhenRecipeExists_DeletesRecipe()
    {
        ...
        await _repository.DeleteRecipeAsync(recipe.Id, TestUserId);
    ...
    {
        var exception = await Assert.ThrowsAsync<Exception>(() => _repository.DeleteRecipeAsync(999, TestUserId));
        exception.Message.Should().Contain("Не найден рецепт");
    }
    ...
    // Дополнительный тест: проверка ошибки при попытке изменения чужого рецепта
    [Fact]
    public async Task UpdateRecipeAsync_WhenDifferentUser_ThrowsException()
    {
        var recipe = new RecipeModel
        {
            Name = "Original Recipe",
        ...
        var exception = await Assert.ThrowsAsync<Exception>(() => _repository.UpdateRecipeAsync(recipeUpdate, TestUserId));
        exception.Message.Should().Contain("Нельзя изменить рецепт, созданный другим пользователем");
    }

    // Дополнительный тест: проверка ошибки при попытке удаления чужого рецепта
    [Fact]
    public async Task DeleteRecipeAsync_WhenDifferentUser_ThrowsException()
    {
        var recipe = new RecipeModel
        {
            Name = "Test Recipe",
        ...
        var exception = await Assert.ThrowsAsync<Exception>(() => _repository.DeleteRecipeAsync(recipe.Id, TestUserId));
        exception.Message.Should().Contain("Нельзя удалить рецепт, созданный другим пользователем");
    }
